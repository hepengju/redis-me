export default {
  ...
  setting: {
    ...
    connectTimeout: '连接超时',
    connectTimeoutTip:
      '建立 Redis 连接（含 SSH/代理握手、TCP、PING）的最长等待时间；修改后需重连生效。范围 {min}–{max} 秒',
    commandTimeout: '读写超时',
    commandTimeoutTip:
  ...
  conn: {
    ...
    sshProxyMutex: 'SSH 与代理不能同时开启',
    proxyTip: `通过 HTTP / SOCKS 代理连接 Redis，适用于以下场景<br/>• 公司网络只允许走指定代理<br/>• 本机已有 Clash 等系统/环境变量代理<br/>• 主机名需要在代理侧解析（SOCKS5H）<br/>• 与 SSH 互斥；支持单机、集群、哨兵<br/>• Linux 系统代理只认环境变量`,
    proxyOption: {
      config: '配置',
      host: '主机',
      port: '端口',
    ...
    proxyDetectFound: '检测到 {type} {host}:{port}（{source}）',
    proxyDetectFoundAuth: '检测到 {type} {host}:{port}（{source}，含认证）',
    proxyDetecting: '正在检测系统代理…',
    proxyDetectNone: '未检测到系统代理，连接时将直连',
    proxySourceEnv: '环境变量',
    proxySourceWindows: 'Windows 系统代理',
    proxySourceMacos: 'macOS 系统代理',
    loginType: '登录',
    sshOption: {
      host: '主机',
      port: '端口',
...
<b>失败时</b>：优先展示 stderr；退出码非 0 则提示执行失败`,
  ...
  fieldAdd: {
    ...
    zsetHint: '(值：分数)',
    streamHint: '(字段：值)',
    timeseriesHint: '(时间戳：值；空则 *；可填毫秒或日期时间；新建空键为 TS.ADD * 0)',
    arrayHint: '(索引：值)',
    arrayInsertHint: '(值)',
    arrayIndex: '索引',
    ...
    timestamp: '时间戳 (* / 毫秒 / 日期时间)',
    tsValue: '数值',
    tsValueRequired: '请输入样本数值',
    newKey: '新增键',
    newField: '新增字段',
    key: '键',
    field: '字段',
  ...
  keyTree: {
    ...
    unfavoriteKey: '取消收藏',
    favoriteFolder: '收藏目录',
    unfavoriteFolder: '取消收藏',
    reloadFolder: '重新加载',
    reloadAllFolder: '重载全部',
    favoriteOk: '已收藏',
    unfavoriteOk: '已取消收藏',
  },
...
② 正常情况下无需指定节点，仅在查看特定节点配置等特殊场景可手动指定节点
  ...
  redisValue: {
    ...
    arLastItems: 'LastN',
    arLastItemsTitle: 'Array 最近插入',
    arLastItemsAsc: '插入序',
    arLastItemsDesc: '最近优先',
    arLastItemsFilter: '过滤值',
    arLastItemsEmpty: '暂无数据',
    arLastItemsNil: '(nil)',
    tsChartTitle: '折线图',
    tsChartSeries: '数值',
    tsChartEmpty: '暂无样本可绘制（请先浏览或加载数据）',
    zsetRangeCount: '数量',
    zsetRangeAsc: '正序',
    zsetRangeDesc: '倒序',
    zsetRangeQuery: '查询',
    ...
    vSimQuery: '查询',
    vSimFilter: '本地筛选',
    vSimFilterHint: 'FILTER，如 .year >= 2000',
    vSimEpsilon: 'EPSILON',
    vSimEf: 'EF',
    vSimEpsilonInvalid: 'EPSILON 须为数字',
...
一旦执行的操作导致 Redis 无法保留节省空间的编码，所有特殊编码类型都会自动转换为通用类型`,
  ...
  keyMain: {
    ...
    clearHistory: '清空历史记录',
    myFavorites: '进入收藏模式',
    favoriteFilter: '搜索收藏目录或键',
    favoriteMode: '收藏模式',
    exitFavoriteMode: '退出收藏',
    favoriteFolders: '收藏目录',
    favoriteKeys: '收藏键',
    favoriteFolderEmpty: '右键文件夹可收藏目录',
    favoriteChecked: '批量收藏',
    unfavoriteChecked: '批量取消收藏',
    favoriteCheckedOk: '已收藏 {count} 个键',
    unfavoriteCheckedOk: '已取消收藏 {count} 个键',
    unfavoriteCheckedFoldersOk: '已取消收藏 {count} 个目录',
    unfavoriteCheckedMixedOk: '已取消收藏 {keyCount} 个键、{folderCount} 个目录',
    favoriteCheckedAllAlready: '所选键均已收藏',
    unfavoriteCheckedNoneAlready: '所选内容均未收藏',
    clearFavorites: '清空收藏',
    clearFavoritesConfirm: '确定清空当前库的全部收藏（键与目录）吗？',
  ...
  errors: {
    ...
    ssh_and_proxy_mutually_exclusive: 'SSH 与网络代理不能同时开启',
    proxy_mode_not_supported: '不支持的代理模式: {mode}',
    proxy_type_not_supported: '不支持的代理类型: {proxy_type}',
    proxy_host_required: '请填写代理主机和端口',
    proxy_auth_required: '代理需要认证',
    proxy_connect_rejected: '代理拒绝 CONNECT（{status}）。部分网关只允许 CONNECT 到 443',
    proxy_handshake_failed: '代理握手失败: {detail}',
