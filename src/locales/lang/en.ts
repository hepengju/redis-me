export default {
  ...
  setting: {
    ...
    connectTimeoutTip:
      'Max wait to establish a Redis connection (SSH/proxy handshake, TCP, PING); takes effect after reconnect. Range {min}–{max} s',
  ...
  conn: {
    ...
    sshProxyMutex: 'SSH and proxy cannot be enabled together',
    proxyTip:
      'Connect to Redis via HTTP/SOCKS proxy<br/>• Corporate networks that require a proxy<br/>• Local Clash / env / OS proxy<br/>• Hostname resolved by the proxy (SOCKS5H)<br/>• Mutually exclusive with SSH; works with standalone, cluster, and sentinel<br/>• On Linux, system proxy uses environment variables only',
    proxyOption: {
      config: 'Config',
      host: 'Host',
      port: 'Port',
      ...
      typeSocks5: 'SOCKS5',
      typeSocks5h: 'SOCKS5H',
      hostRequired: 'Please input proxy host',
      portRequired: 'Please input proxy port',
    },
    proxyDetectFound: 'Detected {type} {host}:{port} ({source})',
    proxyDetectFoundAuth: 'Detected {type} {host}:{port} ({source}, authenticated)',
    proxyDetecting: 'Detecting system proxy…',
    proxyDetectNone: 'No system proxy detected; will connect directly',
    proxySourceEnv: 'environment variables',
    proxySourceWindows: 'Windows system proxy',
    proxySourceMacos: 'macOS system proxy',
    loginType: 'Login',
...
<b>On failure</b>: stderr is shown first; non-zero exit code shows an exec error`,
  ...
  fieldAdd: {
    ...
    timeseriesHint: '(Timestamp: Value; empty = *; ms or datetime OK; empty key → TS.ADD * 0)',
    arrayHint: '(Index: Value)',
    arrayInsertHint: '(Value)',
    arrayIndex: 'Index',
    ...
    timestamp: 'Timestamp (* / ms / datetime)',
    tsValue: 'Number',
    tsValueRequired: 'Sample value is required',
    newKey: 'New Key',
    newField: 'New Field',
    key: 'Key',
    field: 'Field',
    ...
    /** Vector Set: only the element name goes through wire encoding */
    elementCodec: 'Element Codec',
...
② Usually there is no need to specify a node. Manual node specification is only required in special scenarios such as viewing the configuration of a specific node
  ...
  redisValue: {
    ...
    zsetScoreMin: 'Min score',
    zsetScoreMax: 'Max score',
    tsMin: 'From timestamp',
    tsMax: 'To timestamp',
    tsMinValue: 'Min value',
    tsMaxValue: 'Max value',
    timestamp: 'Timestamp',
    listSortAsc: 'Asc',
    listSortDesc: 'Desc',
    ...
    fieldExactSearchVectorSet:
      '<b>Scan</b> (unchecked)<br/>Local filter by element only (VRANGE has no server-side MATCH)<br/><br/><b>Exact</b> (checked)<br/>VISMEMBER + VEMB by element: exact match against the element',
    insertRow: 'Insert',
    ...
    vSimTitle: 'Vector Similarity Search',
    vSimQuery: 'Query',
    vSimFilter: 'Local filter',
    vSimFilterHint: 'FILTER, e.g. .year >= 2000',
    vSimEpsilon: 'EPSILON',
    vSimEf: 'EF',
    vSimEpsilonInvalid: 'EPSILON must be a number',
    ...
    arLastItemsDesc: 'Newest first',
    arLastItemsFilter: 'Filter value',
    arLastItemsEmpty: 'No data',
    arLastItemsNil: '(nil)',
    tsChartTitle: 'Line Chart',
    tsChartSeries: 'Value',
    tsChartEmpty: 'No samples to plot (browse or load data first)',
...
Once an operation prevents Redis from keeping the space-saving encoding, special encodings are automatically converted to the general type`,
  ...
  keyMain: {
    ...
    favoriteFilter: 'Filter folders or keys',
    favoriteMode: 'Favorite Mode',
    exitFavoriteMode: 'Exit Favorites',
    favoriteFolders: 'Favorite Folders',
    favoriteKeys: 'Favorite Keys',
    favoriteFolderEmpty: 'Right-click a folder to favorite it',
    ...
    unfavoriteCheckedFoldersOk: 'Unfavorited {count} folders',
    unfavoriteCheckedMixedOk: 'Unfavorited {keyCount} keys and {folderCount} folders',
    favoriteCheckedAllAlready: 'All selected keys are already favorited',
    unfavoriteCheckedNoneAlready: 'None of the selected items are favorited',
    clearFavorites: 'Clear Favorites',
    clearFavoritesConfirm: 'Clear all favorites (keys and folders) in current DB?',
  ...
  errors: {
    ...
    ssh_and_proxy_mutually_exclusive: 'SSH and network proxy cannot be enabled together',
    proxy_mode_not_supported: 'Unsupported proxy mode: {mode}',
    proxy_type_not_supported: 'Unsupported proxy type: {proxy_type}',
    proxy_host_required: 'Proxy host and port are required',
    proxy_auth_required: 'Proxy authentication required',
    proxy_connect_rejected:
      'Proxy rejected CONNECT ({status}). Some gateways only allow CONNECT to port 443',
    proxy_handshake_failed: 'Proxy handshake failed: {detail}',
