...
- **新增连接**：支持SSH、代理、SSL、只读、集群、哨兵等配置，测试连接
  - **SSH**：SSH隧道模式适用于Redis服务器在内网，无法直接访问，需要通过跳板机访问；**单机 / 集群 / 哨兵**均可使用
  - **代理**：公司出口或本机 Clash 等 HTTP / HTTPS / SOCKS5 / SOCKS5H 代理；也可「使用系统代理」（环境变量与 Windows/macOS 静态设置；Linux 只认环境变量）。与 SSH 互斥。集群发现出的内网 IP 若代理连不上，请改用 SSH。
